...
- [x] ساخت جدول تاریخ (Calendar Table)
- [x] مدل‌سازی پایه و Relationship
- [x] نوشتن DAX Measures پایه (Total Revenue, Total Orders, Total Customers, AOV, Return Rate)
- [x] صفحه Overview
- [x] صفحه Products
- [x] صفحه Customers (با چند باگ باز، پایین‌تر لیست شده)
- [x] رفع باگ عدد `Purchase Frequency` (فرمول اصلاح شد؛ عدد درست ≈ 4.27)
- [x] اصلاح کارت چهارم تکراری (Average Order Value) در صفحه Customers
- [x] اصلاح غلط املایی "Top 10 custometr"
- [x] اتصال درست Target در KPI به `Revenue per Customer PM` (با ساخت `MonthStartDate`)
- [ ] ساخت Slicer برای `Year` (Tile) و `Country` (Dropdown) در صفحه Customers
- [ ] معیار New vs Returning Customers (`Returning Customers`, `% Returning Customers`)
- [ ] هم‌اندازه و هم‌تراز کردن کارت‌های KPI در همه‌ی صفحات (Format → Align/Distribute)
- [x] صفحه ۴ — Returns: تحلیل برگشتی‌ها
- [x] ساخت تم رنگی سفارشی Navy (`OnlineRetail_NavyTheme.json`) و اعمال به همه‌ی صفحات
- [x] اصلاح نقشه‌ی Overview (جایگزینی با Bar Chart کشورها)
- [x] اصلاح کارت `Total Revenue` صفحه Overview (KPI اشتباه → Card ساده)
- [x] بازگردوندن فیلتر `IsNonProduct=False` روی صفحه Products
- [ ] استایل‌دهی به جدول صفحه Products (قاب/بردر هماهنگ با تم)
- [ ] مرتب‌سازی جدول صفحه Products بر اساس Total Revenue نزولی
- [ ] افزودن عنوان به جدول صفحه Products
- [ ] بازبینی نهایی غلط‌های تایپی در عنوان صفحات (`Producte s`, `Retrun`)
- [ ] تغییر عنوان‌های پیش‌فرض Visual ها به متن انسانی (در همه‌ی صفحات)
- [ ] تبدیل به Star Schema کامل (اختیاری، برای پورتفولیو ارزشمند)
- [ ] تحلیل RFM برای بخش‌بندی مشتری (اختیاری)
- [ ] افزودن Slicer/Interactivity/Drill-through در بقیه‌ی صفحات
- [ ] تست و Cross-check نهایی اعداد
- [ ] نوشتن معرفی پروژه برای GitHub/LinkedIn (پورتفولیو)